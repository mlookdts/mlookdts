...
<div>
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <button 
                    ...
                    class="flex flex-col items-center justify-center p-8 border-2 border-gray-200 dark:border-gray-700 rounded-xl cursor-pointer transition-all hover:scale-[1.02] bg-white dark:bg-gray-800"
                    id="theme-light-btn"
                >
                    <div class="w-20 h-20 bg-gradient-to-br from-yellow-50 to-orange-50 border-2 border-gray-200 rounded-2xl mb-4 flex items-center justify-center shadow-lg">
                        <x-icon name="sun" class="w-10 h-10 text-yellow-500" />
                    </div>
                    <span class="text-base font-semibold text-gray-900 dark:text-white mb-1">Light Mode</span>
                    <span class="text-xs text-gray-500 dark:text-gray-400">Clean and bright interface</span>
                </button>
                ...
                <button 
                    ...
                    id="theme-dark-btn"
                >
                    <div class="w-20 h-20 bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-gray-700 rounded-2xl mb-4 flex items-center justify-center shadow-lg">
                        <x-icon name="moon" class="w-10 h-10 text-blue-400" />
                    </div>
                    <span class="text-base font-semibold text-gray-900 dark:text-white mb-1">Dark Mode</span>
                    <span class="text-xs text-gray-500 dark:text-gray-400">Easy on the eyes</span>
                </button>
            </div>
        </div>
        ...
        <!-- PWA Install Section -->
        <div class="bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-6">
            <div class="flex items-start justify-between">
                <div class="flex items-start flex-1">
                    <div class="w-12 h-12 bg-gradient-to-br from-orange-500 to-amber-600 dark:from-orange-600 dark:to-amber-700 rounded-xl flex items-center justify-center mr-4 flex-shrink-0 shadow-lg dark:shadow-orange-900/50">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                        </svg>
                    </div>
                    <div class="flex-1">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Install as App</h3>
                        <p class="text-sm text-gray-700 dark:text-gray-300 mb-4">
                            Install DTS as a Progressive Web App for quick access, offline support, and a native app experience.
                        </p>
                        <div class="flex flex-wrap gap-2 text-xs text-gray-600 dark:text-gray-300 mb-4">
                        ...
                        <button id="pwa-install-btn" onclick="installPWA()" 
                                class="px-6 py-3 bg-gray-900 hover:bg-gray-800 dark:bg-gray-800 dark:hover:bg-gray-700 text-white rounded-lg font-medium transition-all shadow-lg dark:shadow-gray-900/50 hover:shadow-xl dark:hover:shadow-gray-900/70 transform hover:-translate-y-0.5">
                            <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path>
                            </svg>
                            Install Now
                        </button>
                        <p id="pwa-installed-msg" class="hidden text-sm text-green-600 dark:text-green-400 font-medium">
                            ✓ App is already installed!
                        </p>
                        <p id="pwa-not-supported" class="hidden text-sm text-gray-500 dark:text-gray-400">
...
function checkPWAInstallStatus() {
    ...
    // Also check if running as PWA by checking if window controls overlay is available
    ...
    // Chrome, Edge, Opera support beforeinstallprompt
    // Safari on iOS uses different mechanism (doesn't fire beforeinstallprompt)
    ...
    // On iOS Safari, PWA installation is done via "Add to Home Screen" in browser menu
    // We can't programmatically trigger it, so we show instructions
    ...
    // For Chromium-based browsers, wait for beforeinstallprompt event
    if (isChromium || isEdge || isOpera) {
        ...
        // Listen for beforeinstallprompt event (might fire before this code runs)
            ...
            if (window.deferredPrompt) {
                ...
                if ('serviceWorker' in navigator) {
                    navigator.serviceWorker.getRegistration().then(registration => {
                        ...
                        hideInstallButton();
                        notSupported.textContent = 'Unable to check service worker status.';
                    });
                } else {
                    hideInstallButton();
        ...
        // Browser doesn't support programmatic PWA installation
        hideInstallButton();
        if (isSafari && isMobile) {
            notSupported.textContent = 'On iOS Safari, use the Share button and select "Add to Home Screen"';
        } else {
